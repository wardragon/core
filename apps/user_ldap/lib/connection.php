...
class Connection {
	...
	private function readConfiguration($force = false) {
		...
		if((!$this->configured || $force) && !is_null($this->configID)) {
			...
			$this->config['ldapUserSearchFilter']  = \OCP\Config::getAppValue($this->configID, 'ldap_usersearch_filter', 'uid');
			$this->config['ldapMinSearchLenght']  = \OCP\Config::getAppValue($this->configID, 'min_lenght_search', 0);
	...
	public function setConfiguration($config, &$setParameters = null) {
		...
		'ldap_tls'=>'ldapTLS', 'ldap_nocase'=>'ldapNoCase', 'ldap_quota_def'=>'ldapQuotaDefault', 'ldap_quota_attr'=>'ldapQuotaAttribute', 'ldap_email_attr'=>'ldapEmailAttribute', 'ldap_group_member_assoc_attribute'=>'ldapGroupMemberAssocAttr', 'ldap_cache_ttl'=>'ldapCacheTTL', 'home_folder_naming_rule' => 'homeFolderNamingRule','ldap_usersearch_filter' => 'ldapUserSearchFilter','min_lenght_search' => 'ldapMinSearchLenght');
	...
	private function validateConfiguration() {
		...
		if(empty($this->config['ldapMinSearchLenght'])) {
			$this->config['ldapMinSearchLenght'] = 0;
			\OCP\Util::writeLog('user_ldap', 'min Ldap Search Lenght not specified, will be set to default value 0.', \OCP\Util::INFO);
		}
		...
		if(empty($this->config['ldapUserSearchFilter'])) {
			\OCP\Util::writeLog('user_ldap', 'No user search specified, ldap search could be incomplete.', \OCP\Util::WARN);
			$configurationOK = false;
		}
